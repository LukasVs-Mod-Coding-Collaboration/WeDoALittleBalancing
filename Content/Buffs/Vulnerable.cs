...
namespace WeDoALittleBalancing.Content.Buffs
{
    public class Vulnerable : ModBuff
    {
        ...
        {
            tip = tip+" "+(10+(Main.player[Main.myPlayer].GetModPlayer<WDALBPlayerUtil>().vulnerableStack * 10))+"%";
            base.ModifyBuffText(ref buffName, ref tip, ref rare);
        }
        
        public override bool ReApply(Player player, int time, int buffIndex)
        {
            if(player.GetModPlayer<WDALBPlayerUtil>().vulnerableStack < 3)
            {
                player.GetModPlayer<WDALBPlayerUtil>().vulnerableStack += 1;
            }
