...
namespace WeDoALittleBalancing.Content.Buffs
{
    public class WreckedResistance : ModBuff
    {
        ...
        {
            tip = tip+" "+(10+(Main.player[Main.myPlayer].GetModPlayer<WDALBPlayerUtil>().wreckedResistanceStack * 10))+"%";
            base.ModifyBuffText(ref buffName, ref tip, ref rare);
        }
        
        public override bool ReApply(Player player, int time, int buffIndex)
        {
            if(player.GetModPlayer<WDALBPlayerUtil>().wreckedResistanceStack < 3)
            {
                player.GetModPlayer<WDALBPlayerUtil>().wreckedResistanceStack += 1;
            }
